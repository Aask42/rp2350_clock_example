...
void fm_power_up(rda5807_t *radio, fm_config_t config) {
    ...
    // Validate device presence on the I2C bus
    if (!fm_read_single_register(i2c_inst, regs, 0x0)) {
        printf("Error: FM device not found on I2C bus. Check wiring and address.\n");
...
void fm_set_frequency_blocking(rda5807_t *radio, float frequency) {
    ...
    do {
        ...
        printf("Tuning to %.2f MHz, progress: %d\n", frequency, progress.result);
