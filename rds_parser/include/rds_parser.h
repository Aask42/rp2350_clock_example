 ...
 */
 ...
 typedef enum
 {
    ...
    RDS_GROUP_RT = 0x02,
    RDS_GROUP_CLOCK = 0x04,   // Group 4A for Clock-Time
    RDS_GROUP_RT_PLUS = 0x11, // Group 17A for RadioText Plus
     
 } rds_group_type_t;

 // Add new structure for time information
typedef struct {
    uint16_t year;
    uint8_t month;
    uint8_t day;
 ...
 typedef struct rds_parser_t
 {
     ...
     rds_time_info_t time_info;
     char current_title[64];
     char current_artist[64];
     bool has_rt_plus;
     uint8_t rt_plus_tags[2];
     uint8_t rt_plus_start[2];
     uint8_t rt_plus_length[2];
 #endif
 } rds_parser_t;
